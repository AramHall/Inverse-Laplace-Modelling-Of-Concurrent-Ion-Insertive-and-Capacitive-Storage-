function [ fittedCurrent , fittedParams,ci] = PITTfit_w_C_redo( current, time, bLength, Estep,initParams )
...
opts = optimoptions('lsqnonlin','FunctionTolerance',1e-20,'OptimalityTolerance',1e-15,...
    'MaxIterations', 5000,'MaxFunctionEvaluations',2000,'StepTolerance',1e-20);


% sets solver model, based on the PITT_w_C_redo function
iModel01 =@(fittedParams) PITT_w_C_redo( fittedParams(1), fittedParams(2),...
    fittedParams(3), fittedParams(4), fittedParams(5), bLength, Estep, time ) - current;

% nonlinear least squares to fit the model to PITT data
[fittedParams,resnorm,resid,exitflag,output,lambda,J] = lsqnonlin(iModel01,pInx,[1e-5;1e-5;1e-5;1e-8;1e-1],[100;1000;Inf;1e-2;1000],opts);


% the default root finder, which is less rigerous but far faster, is
% checked against to rigerous rott finder to make sure roots of the fitted
% values were calcuated correctly
if mae(PITT_root_finder_cmplx_redo_rigourous( fittedParams(1),fittedParams(2),...
        fittedParams(3),fittedParams(4),fittedParams(5),bLength)-...
        PITT_root_finder_cmplx_redo( fittedParams(1),fittedParams(2),...
        fittedParams(3),fittedParams(4),fittedParams(5),bLength)) >1
    f = warndlg('b Root calculation is bad - switch to more rigarous method!','Warning')
end
...
fittedParams(3), fittedParams(4), fittedParams(5), bLength, Estep, time  );
